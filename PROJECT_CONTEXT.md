...
The ScreenScraper spike approves a constrained V1 direct-client model. Rust communicates with the
provider using RetroFrontier-owned application credentials injected into release builds outside
source control; those values are recoverable application credentials, not a cryptographic secret
boundary. Optional personal user credentials use the OS vault/keychain. V1 caches normalized
