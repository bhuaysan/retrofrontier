...
12. build strong process identity and atomically replace the record with `running`; on failure the
    child is terminated and the launch fails closed;
...
15. on exit: classify the outcome, close the session, clear the record, publish the stable state.
...
| --- | --- | --- |
| `launching` | must be absent | previous boot ⇒ dead; otherwise a bounded `/proc` scan for an executable inside `runtime/versions/` or an `argv[0]` equal to the authenticated AppRun |
...
| present but uncertain | nothing closed, nothing deleted, launches refused, `blocked` reported |
