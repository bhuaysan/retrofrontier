...
**Status:** complete on `feat/m9-save-states`. Every task below is implemented and verified; the
deviations from this plan are recorded in the final implementation report and in
`docs/SAVE_STATES.md`.
...
- [x] **Step 7:** Add `rustix = { version = "1.1", features = ["fs"] }` to `src-tauri/Cargo.toml` — already resolved transitively at `1.1.4`, so no new supply-chain surface — and implement the adapter.
...
- [x] **Step 1 (RED) — hotkey derivation from authenticated profiles:** Using a synthetic authenticated profile tree, assert `resolve_managed_save_state_hotkeys`
...
- [x] **Step 2 (RED) — generated configuration:** Assert the generated `retroarch.cfg`
...
- [x] **Step 3 (RED) — launch argument contract:** Assert a normal launch's arguments remain exactly `--config <cfg> -L <core> <content>` and that a save-state launch is exactly `--config <cfg> -L <core> --entryslot <slot> <content>`, with the slot rendered as a bare decimal in `1..=999` and the content target still last.
- [x] **Step 4:** Replace the positional `RetroArchConfig::build(paths, core_directory, controller_profiles_root)` with `RetroArchConfig::build(&RetroArchConfigRequest { paths, core_directory, controller_profiles_root, state_slot, save_state_hotkeys })`, extend `LaunchPreparation` with `entry_slot: Option<SaveStateSlot>`, and implement `retroarch_input.rs`. The application-owned list of qualified managed controller device profiles lives in `retroarch_input.rs` as a documented constant referencing the M8 hardware qualification; it is a list of *device profile filenames inside the authenticated database*, never a button table.
- [x] **Step 5:** `state_slot` is written **in addition to** `--entryslot` deliberately. `--entryslot` is the documented mechanism and the argument contract is unchanged; the configuration value makes "the active slot after a save-state launch is the loaded state's own slot" hold deterministically from RetroFrontier's own single control path, which the generated file already is. Note this in `docs/SAVE_STATES.md` and add it to the manual qualification checklist.
...
- [x] **Step 5:** Add the `SaveStateLifecycle` collaborator:
...
- [x] **Step 1 (RED) — reconciliation:** Using a synthetic states tree, a real repository, and a deterministic stability probe, assert reconciliation
...
- [x] **Step 2 (RED) — list and capabilities:** Assert `list_save_states`
...
- [x] **Step 2:** Mirror `SaveStateView`, `SaveStateCapabilities`, `SaveStateLoadability`, `SaveStateErrorCode`, `SaveStateError`, `DeleteSaveStateResponse` in `ipc.ts` and add `listSaveStates`, `loadSaveState`, `deleteSaveState`. Implement `useSaveStates` following the existing `useGameDetail` channel pattern.
...
- [x] `cargo test --manifest-path src-tauri/Cargo.toml` and `cargo test --manifest-path src-tauri/Cargo.toml --all-features`
- [x] `git diff --check`; compare `git status --porcelain --ignored` before and after to prove no pre-existing untracked/ignored artifact changed
- [x] Confirm no secret, ROM, BIOS file, runtime binary, database, log, or generated artifact entered the diff
- [x] `/code-review` at high effort over the branch, then address valid findings through the same TDD loop
- [x] Write the structured implementation report; report every deviation explicitly
